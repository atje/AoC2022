...
func main() {
	...
	for i, line := range lines {
		...
		mid := len(chars) / 2
		for i := 0; i < mid; i++ {
			for j := mid; j < len(chars); j++ {
				if chars[i] == chars[j] {
					if *dbgFlag {
						fmt.Println("Found duplicate: ", string(chars[j]), "i=", i, "j=", j)
					}
					if !strings.Contains(buf.String(), string(chars[j])) {
						c := int(chars[j])
						if c < 96 {
						...
						buf.WriteRune(chars[j])
						if *dbgFlag {
							fmt.Println("New duplicate: ", string(chars[j]), ", new score = ", score, ", c = ", c)
						}
					}
				}
			}
	...
	fmt.Println("Found duplicates:", btot.String(), ", total score ", score)
}
